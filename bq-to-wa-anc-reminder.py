...
class GoogleSheet(Base):
    __gs_sheet_name            = ''
    __gs_report_id             = ''
    __service_account_filename = ''
    
    # ----------------------------------------------------------------------------
    def __init__(self, config_filename='GoogleSheet.cfg'):
        self.__read_config(config_filename)
        gc = gspread.service_account(filename=self.__service_account_filename)
        sh = gc.open_by_key(self.__gs_report_id)

        self.worksheet_list = sh.worksheets()
        self.worksheet = sh.worksheet(self.__gs_sheet_name)

        self.gs_report_list = []

    # ----------------------------------------------------------------------------
    def __read_config(self, config_filename='GoogleSheet.cfg'):
        ...
        for line in fin.readlines():
            ...
            rline = re.search('^GS_SHEET_NAME\s*=\s*"([^\"]+)"', line)
            if rline:
                self.__gs_sheet_name = rline.group(1)
                continue

            rline = re.search('^GS_REPORT_ID\s*=\s*"([^\"]+)"', line)
            if rline:
                self.__gs_report_id = rline.group(1)
                continue

            rline = re.search('^SERVICE_ACCOUNT_FILENAME\s*=\s*"([^\"]+)"', line)
            if rline:
                self.__service_account_filename = rline.group(1)
    ...
    def create_report_list(
        ...
        if self.testing:
            self.report_list.append("Testing")
...
class Qontak(Base):
    ...
    # ----------------------------------------------------------------------------
    def __read_config(self, config_filename='wa_qontak.cfg'):
        fin = open(config_filename)
        for line in fin.readlines():
            rline = re.search('^#', line)
            if rline:
            ...
            rline = re.search('^WA_AUTHORIZATION\s*=\s*"([^\"]+)"', line)
            if rline:
                self.wa_channel_integration_id = rline.group(1)
                continue

            rline = re.search('^WA_CHANNEL_INTEGRATION_ID\s*=\s*"([^\"]+)"', line)
            if rline:
                self.wa_channel_integration_id = rline.group(1)
                continue

            rline = re.search('^HP_TEST_NO\s*=\s*"([^\"]+)"', line)
            if rline:
                self.hp_test_no = rline.group(1)
    ...
    def collect_wa_log_report(self):
        ...
        headers = {"Authorization": self.wa_authorization}

        response = requests.get(
            "https://service-chat.qontak.com/api/open/v1/broadcasts/whatsapp?direction=desc",
            headers=headers,
        )
    ...
    # ----------------------------------------------------------------------------
    def wa_direct_send(self, mobile_number, data=dict(), debug_title, debug_msg):
        if self.testing:
        ...
        try:
            ...
            if self.debug:
                print(f"[debug] SUCCESS {debug_msg} sent to Qontak {debug_title}")

            return f"Successfully sent request: {response.text}", 200
        except requests.exceptions.RequestException as e:
            if self.debug:
                print(f"[debug] ERROR {debug_msg} exception:{str(e)}")
...
class BigQuery(Base):
    ...
    # -----------------------------------------------------------------------------
    def __init__(self, query_filename, message_template_id=""):
        Base.__init__(self, message_template_id)
    ...
    def read_file_and_replace_param_to_query(self, query_filename):
        ...
        self.query = query.replace(
            "{self.message_template_id}", str(self.message_template_id)
        )
    ...
    def get_bq_wa_sent_identifier_list(self):
        query_job = self.client.query(
            f"SELECT identifier FROM `nextgen-398301.fhir_wa.sent_status` \
...
class WA_ANC_Reminder(BigQuery, Qontak, GoogleSheet):
    ...
    # -----------------------------------------------------------------------------
    def __init__(self):
        self.last_visit_week_range = 1
        self.days_before_wa = 1
        message_template_id, big_query_filename, google_sheet_name = self.__read_config(config_filename)
        Qontak.__init__(self, message_template_id)
        BigQuery.__init__(self, big_query_filename)
        GoogleSheet.__init__(self, google_sheet_name)

        self.replace_param_to_query()

        self.today = datetime.today()
        self.today_str = self.today.strftime("%Y-%m-%d")

        self.next_visit_date = (
            self.today + timedelta(days=self.days_before_wa)
        ).strftime("%Y-%m-%d")
    ...
    def __read_config(self, config_filename='wa_qontak.cfg'):
        ...
        for line in fin.readlines():
            ...
            rline = re.search('^WA_MESSAGE_TEMPLATE_ID\s*=\s*"([^\"]+)"', line)
            if rline:
                message_template_id = rline.group(1)
                continue

            rline = re.search('^WA_QUERY_FILENAME\s*=\s*"([^\"]+)"', line)
            if rline:
                big_query_filename = rline.group(1)
                continue

            rline = re.search('^GOOGLE_SHEET_NAME\s*=\s*"([^\"]+)"', line)
            if rline:
                google_sheet_name = rline.group(1)

            rline = re.search('^BQ_LAST_VISIT_WEEK_RANGE\s*=\s*(\d+)', line)
            if rline:
                self.last_visit_week_range = rline.group(1)

            rline = re.search('^BQ_DAYS_BEFORE_WA\s*=\s*(\d+)', line)
            if rline:
                self.days_before_wa = rline.group(1)

        fin.close()

        return message_template_id, big_query_filename, google_sheet_name

    # ----------------------------------------------------------------------------
    def replace_param_to_query(self):
        self.query = selg.query.replace(
            "{self.last_visit_week_range}", str(self.last_visit_week_range)
        )
        self.query = self.query.replace("{self.days_before_wa}", str(self.days_before_wa))

    # ----------------------------------------------------------------------------
    def wa_send_msg(self, mobile_number, customer_name):
        print(f"wa_send_msg('{mobile_number}', '{customer_name}')")
        ...
        data = {
            ...
            "parameters": {
                "body": [
                    {"key": "1", "value_text": customer_name, "value": "customer_name"},
                    {
                        "key": "2",
        ...
        debug_msg = f"customer_name:{customer_name}, next_visit_date:{self.next_visit_date}"
        debug_title = "ANC Reminder"

        self.wa_direct_send(mobile_number, data, debug_msg, debug_title)
    ...
    def test(self):
        ...
        for row in self.get_patient_data():
            total_counter += 1
            print(
                f"{row.Source}|{row.Id}|{row.Identifier}|{row.Name}|{row.Telecom}|{row.District}|{row.City}|{ row.last_mens_date}|{row.gestational_age}|{row.Trimester}|{row.last_visit_date}|{row.next_visit_date}"
            )
    ...
    def execute(self):
        ...
        if self.debug:
            print(
                "[debug] {patient_data.Source}|{patient_data.Id}|{patient_data.Identifier}|{patient_data.Name}|{patient_data.Telecom}|{patient_data.District}|{patient_data.City}|{patient_data.last_mens_date}|{patient_data.gestational_age}|{patient_data.Trimester}|{patient_data.last_visit_date}|{self.next_visit_date}"
            )
        for patient_data in patient_rows:
            ...
            if self.debug:
                print(
                    f"[debug] patient data: {patient_data.Source}|{patient_data.Id}|{patient_data.Identifier}|{patient_data.Name}|{patient_data.Telecom}|{patient_data.District}|{patient_data.City}|{patient_data.last_mens_date}|{patient_data.gestational_age}|{patient_data.Trimester}|{patient_data.last_visit_date}|{self.next_visit_date}"
                )

            if mobile:
                result, finalStatus = self.wa_send_msg(mobile, name)
                ...
                if finalStatus == 500 and mobile2:
                    result, finalStatus = self.wa_send_msg(mobile2, name)
                    self.create_report_list(
                        server_name,
                        patient_id,
                        identifier,
                ...
                if finalStatus == 200:
                    self.create_sent_id_to_bq(
                        patient_id, identifier, name, mobile, "sent"
                    )
                else:
                    self.create_sent_id_to_bq(
                        patient_id, identifier, name, mobile, "failed"
                    )
            ...
            if self.report_list:
                ...
                if self.testing:
                    batch_data["range"] = f"A{row_num}:S{row_num}"
...
### TESTING ###
#  result, finalStatus = myWA_ANC_Reminder.wa_send_msg('087876370598', 'TEST Mother')
